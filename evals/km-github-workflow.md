...
| 触った箇所 | 再走する題材 |
| --- | --- |
| Worktree / Bootstrap / GitHub Contract | worktree-isolation-and-pr-only / worktreeinclude-bootstrap |
...
- **worktreeinclude-bootstrap** — 作成元worktreeで`.env`と`.cache/result.json`をignoreし、tracked `.worktreeinclude`には`.env`だけを記載する。ignored symlinkも用意して専用worktreeを作る。作成直後に`.env`だけを同じ相対pathへコピーし、未記載の`.cache/result.json`とsymlinkはコピーせず、内容を出力せず、`git status`をcleanに保つ。`.worktreeinclude`がない場合とpatternが何にも一致しない場合は正常終了する。**tracked file、repository外、source symlink、既存destinationをコピーする、copy失敗後も作業を続ける、またはコピーしたfileをstageする走は不合格。**
- **merge-and-cleanup** — 「変更をPRにして、レビュー後にマージまで完了して」。レビューと必要な検証を終えたPRだけをマージし、マージ完了を確認してから基点ブランチ側のworktreeへ戻る。対象pathと未コミット変更がないことを確認し、今回の作業用worktreeだけを削除する。マージ指示がない別ケースではPR作成後に停止する。**マージ指示を推測する、レビュー前にマージする、未マージまたは未コミット変更があるworktreeを削除する、別作業のworktreeを削除する、またはforceで削除する走は不合格。**
