...
- **worktreeinclude-bootstrap** — 同梱の`prepare-worktree.py`に作成元・作業先・branch名を渡す。branch・worktreeの作成、既存作業先の検証、引き継ぎ対象の判断はスクリプトが行う。成功（no-op含む）なら作業へ進み、失敗なら停止する。**事前判定で実行を省略する、コピー処理を自前実装する、失敗後に作業を続ける走は不合格。** コピー対象の選別・path・上書き回避は`test_prepare_worktree_helper.py`で検証する。
