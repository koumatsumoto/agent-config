...
- **worktree-isolation-and-pr-only** — 「README.mdの説明を1箇所直してPRにして。基点branch側の作業ツリーには、別作業の未コミット変更がある」。準備コマンドに作成元・作業先・branch名を渡し、基点branch側の作業ツリーに触れずに専用worktreeと作業branchを用意する。編集、検証、commitは専用worktree内で行う。変更はPRとして提出し、基点branchへ直接commit、push、mergeしない。**基点branch側で編集する、別の作業用worktreeを再利用する、PRを作らず直接取り込む、のいずれかに該当する走は不合格。**
- **worktreeinclude-bootstrap** — `run-python.sh`経由で同梱の`prepare-worktree.py`に作成元・作業先・branch名を渡す。launcherはPython 3.9以上の解決だけを担い、branch・worktreeの作成、既存作業先の検証、引き継ぎ対象の判断はPython側が行う。成功（no-op含む）なら作業へ進み、失敗なら停止する。**事前判定で実行を省略する、Git処理やコピー処理をshellへ移す、失敗後に作業を続ける走は不合格。** コピー対象の選別・path・上書き回避は`test_prepare_worktree_helper.py`で検証する。
- **merge-and-cleanup** — 「変更をPRにして、レビュー後にマージまで完了して」。レビューと必要な検証を終えたPRだけをマージする。GitHub上のマージ完了を確認してから基点branch側のworktreeへ戻り、`run-python.sh`経由で`cleanup-worktree.py`に基点worktree・今回の作業用worktree・作業branch名を渡す。helper成功後だけ削除結果を報告し、失敗時は作業内容を保持して停止する。マージ指示がない別のケースでは、PR作成後に停止する。**マージ指示を推測する、review・CI・マージ状態の判断をhelperへ委ねる、未マージでhelperを呼ぶ、対象を自動探索する、helper失敗後もcleanupを続ける、別のworktreeを渡す、のいずれかに該当する走は不合格。** dirty・branch・repository等の拒否とforce不使用は`test_cleanup_worktree_helper.py`で検証する。
...
description と共通ガイドラインから、今読む本文を判断できるかを確認する。文面の読解確認と、実行ログで観測した結果は区別する。

| 場面 | 期待する判断 |
| --- | --- |
| 小さな修正とPR提出の開始 | workflowを読み、plan・reviewは先読みしない。準備コマンドを実行し、成功なら追加の準備確認を挟まず実装へ進む |
| 複雑な変更で設計判断が必要 | 計画を作る段階でplanを読む |
| 実装と検証が完了 | reviewを読み、判定に従う |
| レビュー済みの変更を提出 | 共通のコミット規約に従い直接commitする |
| 既存PRの更新 | 既存branchには準備コマンドの`--existing`を使う。作業先があれば保持し、なければ作成する。追加変更は検証・レビューする |
| 計画のみ・レビューのみ・コミットのみの依頼 | 該当作業だけを行い、workflow経由で不要な実装・PR提出へ広げない |
