...
1. **強度を規模・不可逆性・リスクで選ぶ**（迷ったら一段上へ。subagent 非対応環境は self-review にフォールバック）:
   ...
   - **専門家追加** — 高リスク領域（認証・認可 / 秘密情報 / データ移行 / 破壊的操作 / 不可逆な公開契約 / LLM の tool・入力境界）に触れるとき、該当レンズを必要な分だけ足す
2. **指摘を入力クラス別に反映する**:
...
subagent 起動プロンプト（`<plan skill root>` と計画ファイルは `~` を展開した**絶対パス**に置換して渡す。install root: Claude Code は `~/.claude/skills/plan/`、Codex CLI は `~/.agents/skills/plan/`）:
