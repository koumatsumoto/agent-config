...
- レビュー履歴・採否経緯が plan 本文・issue 本文に残っていない（最終計画・意図的な受け入れ済みリスク・実装時確認事項だけが残る）
...
- 要求そのものが変わった差分を計画 / issue に反映する依頼（例: 「要求が変わったので計画を更新して」「この制約を追加して issue を直して」。issue 化後に別会話で届く場合を含む）は「Re-anchoring（要求差分の伝播）」節に従う
...
4. materialize に進む mode では **Clarify Gate**: 計画の骨子（Goal / Non-goal / 合否判定できる DoD / 主要トレードオフ。各々の定義は plan-template.md）を左右する未決事項を解消する。要件を取り違えていないか・浅い理解で進めていないかを確かめ、**本質を引き出して固めてから**進む
   ...
   - **主要成果物の品質特性**（サイズ・性能・保守性・トークン効率など非機能の良し悪し基準）のうち **計画を左右するもの**を確認観点に含める。品質特性が第一制約になりうる計画でそれを anchor に落とさないと、反証不能な DoD（測定手続きのない受け入れ条件）や materialize 後の要求差分の発生源になる。品質特性が争点にならない軽量計画では聞かない（「分岐する未決だけ確認する」と同じ条件フィルタ）
   - 確認で固めた骨子は、計画冒頭の **Goal / Non-goal / Definition of Done anchor** に落とす（plan-template.md）。未解消のまま Phase 2 に進まない
5. **前提宣言**（materialize 直前、非ブロッキング）: 書き出す前に、置いた前提・スコープ判断・品質特性の解釈を **3〜6 行の 1 ブロック**でユーザに提示する。質問ではなく宣言で、**返答を待たずに Phase 2 へ進む**。宣言に落とせるのは **どの妥当な解を選んでも DoD が変わらない前提だけ**。**DoD / スコープ / 主要トレードオフを左右する不確実性は宣言でなく step 4 の質問（ブロッキング）へ格上げする**（宣言を質問回避の抜け道にしない）。自動連鎖・headless 実行（複合依頼「計画を作って PR まで」等）では宣言を読む人が居らず拒否点が働かないため、この格上げを特に守る。draft-only mode では行わない
...
5. **pre-write DoD self-lint**（gate, Secret Check と並ぶ pre-write gate）: 組み立てた本文の Definition of Done・作業単位に plan-template.md「DoD の self-lint 基準」の 4 点（測定手続きと閾値 / ヘッジ語なし / DoD ↔ 作業単位の双方向参照 / 出典の再確認）を適用する。違反があれば **書き出さず修正**してから通す（判断レビュー（Phase 3）より前に、書き出し前チェックで落とせる欠陥をここで落とす。項目の定義・合否例は plan-template.md に一元化）
6. Secret Check と DoD self-lint を通過したら書き出す（**step 5 が本文を書き換えた場合は、書換後の本文に Secret Check を再適用してから**。内容変更 gate の後に秘密が混入したまま書き出さない）。同日付の既存 `.plan/YYYYMMDD-*.md` と衝突しないか確認し（衝突時はファイル名規約の suffix を付ける。既存計画の上書きを避ける）、`.plan/YYYYMMDD-<slug>.md` に書き出す
...
2. **レビュー強度を計画規模・リスクで選ぶ**（rubric）。2 レンズ（generalist＝見落とし / adversary＝敵対的分析）は全強度で共通。最重要は「計画が**本質的に満たすべき要件（Goal）を取り違えていないか**」を問うことで、観点は `plan-review-checklist.md`。多角的に作り込むのではなく、**強度だけを規模で伸縮**させる:
...
3. 返ってきたものを **入力クラス別**に反映する:
   - **重大度付きの指摘**（generalist / adversary / 専門家レンズが grade したもの）: **① plan 本文へ直接反映** / **② 受け入れ済みリスクとして記録** の 2 択。`CRITICAL` / `HIGH` は ① に収束させる（step 4 で解消まで反復）。② に回せるのは `MEDIUM` / `LOW` のみ。**grade 済みの指摘を「実装時確認事項」へ委譲し直さない**（reviewer が可逆性トリアージで「深掘り＝委譲不可」と判断した結果が重大度付きなので、再委譲は判断の無効化）
   - **重大度なしの「実装時確認事項候補」**（checklist の可逆性トリアージで両方 No＝実装後に詰めるほうが安いと申し送られた項目）: plan 本文の「実装時確認事項」節へ **転記**する（記録・消化規約は plan-template.md）。転記前に **高影響カテゴリ（security / 秘密情報 / 認証・認可 / 不可逆・広範囲 / feasibility・正しさ）に触れないか**だけ orchestrator が確認し、触れるなら重大度付き指摘へ戻す（reviewer の取りこぼしへの二次防御。checklist の高影響ガードと二層で守る）
   - 指摘内容・修正経緯（採否判断・反映の往復）は plan 本文にも issue 本文にも書き戻さない。共有用に履歴を残したい場合は issue 作成後に issue comments を使う（comments の内容を本文へ再同期しない）
4. `CRITICAL` / `HIGH` が解消するまで反復する。**初回はフルレビュー、2 回目以降は前回未解決の `CRITICAL` / `HIGH` の解消確認に絞った差分レビュー**にする（毎回フルで回さず、収束に向けてコストを絞る）。反復は **3 周を上限**とする。重大度付き `MEDIUM` / `LOW` は **① 反映を原則**とし、② 受け入れ済みリスクは意図的に残すものだけ plan 本文に記録する（記録項目は plan-template.md に従う）
...
## 補足
- 計画の前提（コードベースについて「調査で分かった事実」とされている箇所など）が実際に正しいか確認するため、必要なら repo 内のファイルを Read してよい。**出典（`file:line` 等）が付いた事実主張のうち、真偽が設計判断・作業単位の成否を左右する load-bearing なものは直接たどって確認し、その他はサンプル確認、出典の無い事実主張は「判定保留 / 要検証」とする**（事実の全数照合に労力を奪われず、Goal 取り違え・破綻箇所・スコープの判断に集中する）
...
`.plan/` のパス、issue URL、意図的に残した受け入れ済みリスク、**plan 本文の「実装時確認事項」（あれば消化先＝紐付けた作業単位 / 実装 PR での確認、とともに）**、未同期があればその旨をユーザに報告する。計画は **検証可能な作業単位に分解済み**（plan-template.md）なので、issue はそのまま km:github-workflow の実装タスク列として渡り、実装 agent が再分解せず最初の作業単位から着手できる。
...
## Re-anchoring（要求差分の伝播）

materialize 後にユーザの要求・制約が変わった、あるいはレビュー指摘や「調査で分かった事実」の誤りが計画を動かす差分が届いたら、まず **差分が anchor（Goal / Non-goal / DoD）を動かすか**を判定する。動かさない要求変更（既存 DoD の範囲内の軽微な追加・調整など）は、該当作業単位・検証への局所反映＋その範囲の Phase 3 再適用に留める（anchor-first 伝播は不要）。**anchor を動かす差分**は、成果物節や作業単位だけを局所パッチせず、**anchor を先に改版してから下流（設計判断 → 作業単位 → 検証 → リスク）へ順に伝播**させる（下流から入れると宙に浮いた DoD・作業単位が残る）。削除・変更した DoD を参照する作業単位・検証が残っていないか、新 DoD に担当作業単位があるかを点検し、変更範囲に Phase 3 のレビューを再適用する（anchor が変われば変更範囲のフルレビュー）。**再レビューが収束（未解決 CRITICAL / HIGH ゼロ）してから**、issue 化済みなら pre-issue Secret Check を通し、Phase 4「既存 issue 更新の例外」の手順で re-sync する（収束しなければ Phase 3 step 5 と同じく更新を止めユーザに委ねる。**未収束のまま公開 issue を更新しない**）。anchor に触れないレビュー指摘の個別管理は次節 `Incorporating External Review Feedback` が扱う。

## Incorporating External Review Feedback

ユーザが外部レビュー結果を持ち込んだ場合（複数レビュアー・指摘多数を含むことがある）、「まとめて確認」して漏らす事故を避けるため次を行う。

指摘が anchor（Goal / Non-goal / DoD）を動かす場合（例: 外部レビューの「Goal を取り違えている」級の指摘）は、個別の局所反映をせず `Re-anchoring` 節の anchor-first 伝播で処理する（局所パッチは宙に浮いた DoD・作業単位を招くため）。以降の手順は anchor に触れない指摘に適用する。
...
2. **対応要否を個別判断**: タスクごとに対応価値を分析する。`CRITICAL` / `HIGH` は原則反映。`MEDIUM` / `LOW` は **orchestrator が可逆性トリアージ（checklist の 2 軸 + 高影響ガード）を適用**し、実装後に詰めるほうが安く高影響でないものを **③ 実装時確認事項へ委譲**（手戻り小の根拠 1 行）、それ以外を ① 反映 / ② 受け入れ済みリスク に個別判定する（外部レビューは km:plan の checklist トリアージを通っていないため、内部指摘と違い orchestrator がここで同じ基準を適用する。記録先は Phase 3 step 3 と同一）
