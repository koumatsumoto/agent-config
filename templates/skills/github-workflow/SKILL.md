...
- `.kaizen/` に記録した改善点を km:kaizen の Report 時 triage で片付ける（`pr` は同 PR で対応済み、`repo` は follow-up issue 化、`workflow` は残置と件数、`knowledge` は fold 先で振り分け）。PR URL・変更要約・検証結果とあわせて triage の結果を報告する。改善点がゼロなら「改善点: なし」の類は書かない
