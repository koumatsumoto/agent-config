...
worktree作成後、同梱スクリプトでローカルファイルの引き継ぎを準備する（`<skill-directory>`はこの`SKILL.md`のあるdirectory）。

```text
python3 "<skill-directory>/scripts/prepare-worktree.py" "<source-root>" "<destination-root>"
```

引数は作成元・作成先worktreeのroot。成功したら実装へ進み、失敗したら停止する。
