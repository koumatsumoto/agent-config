...
- **km-review を起動する条件**: 高影響領域（security / 認証・認可 / 秘密情報 / データ移行・削除 / 不可逆操作 / 公開契約・スキーマ / 広範囲な設計変更）に触れる、ユーザーが明示的にレビューを依頼した、重要な不確実性が残る。高影響かは列挙への字面一致でなく**影響の性質**（不可逆性・攻撃面・信頼境界・波及範囲）で判定し、迷ったら起動する
