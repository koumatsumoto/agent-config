...
- **不変条件継承 probe (無条件・本節が定義)**: 新設・分岐した経路 (新しい関数・エンドポイント・設定分岐・手順) が、既存の同種経路が通す gate・検証・権限・エラー処理を継承しているかを **経路列挙** で確認する。継承漏れ (新経路が既存の安全確認を素通り) は頻出の欠陥クラス (新エンドポイントが既存の認可 middleware を継承しない等)。この probe は **レベル・変更構成・Phase 3 起動有無によらず無条件** — `quick` では本 probe のみ、`test-or-config-or-chore-only` の Skip (Step 1 深度表) からも本 probe のみ除外する (CI 権限・デプロイ等の config 変更こそ継承漏れが刺さる)。`experts/adversary.md` は本定義を主観点として参照する。
- **diff 外との照合 (drift)**: diff が変える識別子・値・契約 (path・設定キー・定数・enum・スキーマ列・エラーコード・公開契約) を diff 内で正しいと仮定せず、権威ある定義元・複製 / 連動箇所 (config・スキーマ・呼び出し元・sibling 実装・doc・テスト) を開いて照合し、片側だけ更新して stale になる drift を拾う。
