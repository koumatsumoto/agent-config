...
正当な利用者・運用者が事故・不可逆損失を起こしにくいか (攻撃者視点はセキュリティ側)。
...
- [ ] kill switch (事故発生時に瞬時に機能停止できる経路) が用意されているか
- [ ] 不可逆操作 (アカウント削除など) にクールダウン期間 / grace period があるか
...
事故防止のための冗長確認 / 利用者通知観点 (利用者エラー回避視点は 4-インタラクション能力 / ユーザーエラー防止側、環境誤認は Risk Identification 側)。

- [ ] 破壊的操作の事故防止確認 (確認 / dry-run / 差分プレビュー) が冗長に成立しているか
- [ ] 破壊的操作の影響範囲 (件数 / 金額 / ユーザ数) をユーザに事前提示するか
...
- [ ] LLM 出力をユーザ意思決定に直接反映する経路で、出典提示 / 信頼度表示 (Overreliance 対策) があるか
