...
- 対象層: Phase 1c（リスク評価と内部深度の導出）・Phase 3 起動判断・結果契約
...
- 判定: 適応型深度の採用根拠（2026-07-18, #142）。常時フル構成の変異は architect 空振り・計量なしの過剰儀式で不採用。トリガ語限定の狭い description は (ii) 検出漏れと「セキュリティ観点で徹底的に見て」の under-trigger を示した
