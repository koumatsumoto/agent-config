...
- **前提・不変条件の攻撃**: 変更が暗黙に置いている前提 (入力の形・呼び出し順・状態・並行度) を崩す入力・経路を探す。特に **不変条件継承 probe** (新設・分岐した経路が既存の同種経路の gate・検証・権限・エラー処理を継承しているかを経路列挙で確認。継承漏れ = 新経路が既存の安全確認を素通り、は頻出の欠陥クラス。定義は `<review skill root>/code-review.md` Step 2 と共通) を主観点として当てる
