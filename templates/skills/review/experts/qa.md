...
**異常系・境界・運用品質**。機能の振る舞いの正しさ、特に「正常系では通るが異常系・境界条件で壊れる」「テストでは見えづらいが本番で問題化する」パターンを見る。

## Phase 2 との住み分け

Phase 2 は「関数単体のエッジケース」(空入力 / null / off-by-one) を拾う。QA は以下の **広い視点** で深掘りする:
...
Phase 2 が同観点を MEDIUM で確定済みなら、QA は「複数経路の連鎖」または「運用での再現条件」が加わる場合のみ補強指摘する。
