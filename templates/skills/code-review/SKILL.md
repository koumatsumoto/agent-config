...
機能適合性以外の品質を確認する。詳細なパターンは review-patterns.md を参照。

### セキュリティ

信頼境界に触れる変更を優先的に確認する。詳細は security-checklist.md を参照。

- シークレットのハードコード、機密情報のログ出力
- 認証・認可の欠如
- 未検証の外部入力（SQL, テンプレート, シェル, パス結合）
- 例外握り潰しによる fail-open

### 信頼性

- 例外の握り潰し・不適切な伝播（cause チェーンの欠落）
- リソース解放漏れ（ファイル、接続、トランザクション）
- 並行アクセスでの整合性破壊（TOCTOU、共有状態）
- 外部サービス障害時の振る舞い（タイムアウト、リトライ）

### 性能効率性

- ホットパスの O(n²) 以上のアルゴリズム
- N+1 クエリ、不要な繰り返しリクエスト
- 無制限に成長するキャッシュ・キュー・バッファ

### 保守性

- 命名の明確さ、ネストの深さ、ロジックの重複
- テストが diff の変更を実際に検証しているか（形だけでないか）
- テストのエッジケースカバレッジと独立性
