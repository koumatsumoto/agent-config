...
blockerとレビュー基準は同一セッションの会話か、直前の一時ディレクトリの`integration.md`から取得する。対象不一致、blockerを特定できない、またはセッションをまたいで一時ファイルが使えない場合は通常レビューへ戻す。`--previous-integration`が存在しない、regular fileでない等の理由でhelperが利用不能と判定した場合も、同じtargetから`--recheck`と`--previous-integration`を除いて準備を1回だけやり直し、通常レビューへ切り替える。
