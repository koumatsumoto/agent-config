...
**main が先に反証・修正・再検証し、その最終候補に残る material risk にだけ独立レビュアを 0〜2 名使う。** 独立性は同じ差分を二重に読むためではなく、main が見落としたものを別コンテキストで拾うために使う。
...
既定は未コミット差分。無ければ current branch の PR diff、それも無ければ `NOOP`。コミット範囲・PR・サブツリーの指定は起動時の指示を解釈する。
...
既存の issue・完了条件・ユーザ指示を基準点として信頼し、要件を再導出・拡張しない。
...
2. **直す** — writable な通常開発では、明確な in-scope finding を修正して関連検証を再実行する。read-only や外部 PR で直せないときも**順序は変えない**
...
**2 名（例外）** — 異なる 2 role に属する material risk がそれぞれ具体化し、片方のレビューが他方を代替できないときだけ。role ごとに選択理由を 1 行残す。「大規模」「重要」だけを理由にせず、同じ汎用レビューを二重に走らせない。
...
- 結果は repo root の `.km-review/<scope-slug>/`（引数なしは `.km-review/uncommitted/`。直下へ平坦化しない）へ残す。**セッションをまたいで発見できる固定パス**であることが recheck の要件。書き込み前に、git 追跡済みならユーザへ確認して止まり、未無視なら `.git/info/exclude` へ `/.km-review/` を追記する（committed `.gitignore` を汚さない）。これらを `git add` しない
- 最終候補が外部から観測される面（挙動・公開契約・設定・コマンド・運用手順）または文書自体を変えるなら、final `PASS` の前に `references/doc-review.md` を main で実施する。`BLOCKED` なら defer してレポートに明記する
