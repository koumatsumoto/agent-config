...
Phase 1 のリスク評価で該当した専門家を起動する（`docs-only` / `test-or-config-or-chore-only` では起動しない）。**軸から owner への写像**（誰を起動するか。対象領域の定義は Phase 1 のリスク評価が正本）: 不可逆性 → architect / 攻撃面 → security + adversary。データ移行は不可逆性軸に属するが、データの移動・削除を伴うため owner は security + adversary とする。`quick` / `standard` からの昇格時は理由 1 行を統合レポートに記録する。
