...
**独立レビュアを走らせる → 自分で読む → 統合して判定する → ドキュメントを見る。** レビュアの選択は前節で済んでいるので、自分の読解を待たせず先に並列起動し、走らせている間に自分で読む。
...
`docs-only` 以外では常に、`references/generalist-review.md` に従って main context で読む。ここで出た指摘も他レビュアと同じ重大度尺度で扱う。**CRITICAL / HIGH が出ても早期停止せず**、起動した全レビュアの完了を待って統合で判定する。読解で新たな高リスク軸が見えたら、この時点からでも該当レビュアを追加起動してよい（昇格理由 1 行をレポートに残す）。
