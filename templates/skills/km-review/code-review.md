...
`AGENTS.md` / `CLAUDE.md` / repo ルールの実質的制約 (重点はコード規約。設計方針・アーキ判断との不整合が見えたらそれも出す)、変更ファイル内の既存コメント / TODO、意図が伝わる命名・不要な複雑性。出さないものは Step 4 で決める。

## Step 4: 偽陽性フィルタ (出さない)

`experts/report-format.md` の偽陽性フィルタを適用する。本 Phase 固有の追加は「シニアレビューとして些末なもの」。
