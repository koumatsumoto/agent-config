...
まず対象 repo の実際の runtime・依存・既存コードのイディオムに合わせる。本ルールの前提バージョンや推奨は想定上限であり、対象環境が対応していない機能は使わない。
...
- 既存 repo がライブラリ・データモデル・インターフェースの方針を定めていればそれに従う。未定の場合の既定として以下を推奨する
