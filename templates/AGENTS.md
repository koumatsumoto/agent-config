...
- コードコメント（docstring / TODO を含む）に残す情報は、後続作業者が保守判断に使う仕様制約・設計判断・機能変更の背景や意図に絞る。レビューラウンド、ユーザとのやり取り、作業中の暫定判断などのローカルな作業履歴は残さない
- issue / PR 番号をコメントに残す場合も、番号だけを根拠にせず、必要な判断理由をコメント内に書いたうえで補助情報として参照する
- コミットメッセージはコードコメントより作業背景を書いてよいが、逐次の作業ログやレビュー反映履歴は入れず、最終差分を理解するための背景・判断・変更内容に絞る
