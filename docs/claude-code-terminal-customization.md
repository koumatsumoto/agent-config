...
上記はテンプレート (`templates/settings.json`) の値で、POSIX シェル（Linux / macOS / Git Bash / WSL2）向け。`install.sh` は OS に応じて `command` を書き換える: POSIXでは `~` パス + shebang のまま起動し、ネイティブ Windows（`cmd.exe`）では `~` 展開も `.py` 直接実行もできないため、インストール時の Python を明示する形式 `"C:/.../python.exe" "C:/Users/.../.claude/statusline.py"` に置換する。
...
- **Python 3.9+**: 標準ライブラリのみで動作。`jq` 等の外部依存なし（JSON は Python でパース）
...
|`effort.level`|推論強度 (low/medium/high/xhigh/max)|
...
|`context_window.current_usage`|キャッシュの読み書き内訳（キャッシュ率算出に使用）|
...
|`pr.number` / `pr.url`|現ブランチの未完了のPR（クリック可能リンク化）|
|`worktree.branch`|Git ブランチ名（ワークツリーのセッション時）|
...
有効化は `/config` → Output style（選択は local レベル = プロジェクトの `.claude/settings.local.json` に保存）か、settings.json の `outputStyle` キー（user / project / local）で行う。styleはセッション開始時に一度だけ読み込まれ、変更の反映には `/clear` または新セッションが必要。
...
許可リストに含めるコマンドは、組み合わせによる攻撃チェーンを考慮する:
...
拒否リストはベストエフォートの追加防御であり、次の制約がある:

- **パターンマッチ**: `*` はワイルドカードとして任意の位置に配置可能だが、`Bash(rm -rf *)` は `rm --recursive --force` にはマッチしない
...
- `Read(/tmp/**)` は避ける: `/tmp` は全ユーザー書き込み可能。シンボリックリンク経由で任意ファイルが読み取られる
