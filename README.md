...
配布CLI、Skill helper、HTMLテンプレートのrender・CSP、文書の参照・一覧を検査するunittest（環境依存の項目は該当OSで実行）:
...
km-planのNode helperは次を実行する。

```bash
node --test scripts/tests/test_prepare_plan_helper.js
```
